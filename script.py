...
def num_to_text(num, main_units=((u'', u'', u''), 'm')):
...
def decimal_to_text(value, places=2, int_units=(('', '', ''), 'm'), exp_units=(('', '', ''), 'm')):
    q = decimal.Decimal(10) ** -places
    integral, exp = str(value.quantize(q)).split('.')
    return u'{} {}'.format(
        num_to_text(int(integral), int_units),
        num_to_text(int(exp), exp_units))
